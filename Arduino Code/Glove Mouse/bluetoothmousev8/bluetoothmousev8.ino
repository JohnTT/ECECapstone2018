...
const float flexThreshold[NUM_FLEX] = {1.8, 2.5, 2.0, 2.9};
...
enum flexStateNames {
  MODE_MOVING = B0000,
  MODE_LEFTCLICK = B1000,
  MODE_RIGHTCLICK = B0100,
  MODE_CALIBRATION = B0010,
  MODE_POINTING = B0111,
...
const double noiseDelta = 0.15; // Reject Differences Below This
...
void loop(void)
{
  ...
  // Pointing: State = B0111
    ...
    if (true) {
      ...
      pointX = (getStep(lastX, euler.x())) * (RESOLUTION_X / (PI / 4));
      pointY = (getStep(lastY, euler.z())) * (RESOLUTION_Y / (PI / 4));
