...
const float flexThreshold[NUM_FLEX] = {2.58, 2.62, 2.60, 2.85};
...
void loop(void)
{
  ...
  if (sheetPressed) {
    ...
    // Right Click: State = Bx1xx
      ...
      analogWrite( PIN_MOTOR , Motor_PWM_DutyCycle / 100.0 * 255 ); // 2% Duty Cycle
    ...
    // Moving: State = Bxxxx
      ...
      if (!isNoise(euler.x(), lastX) || !isNoise(euler.z(), lastY)) {
        ...
        pointX = (getStep(lastX, euler.x())) * (RESOLUTION_X / (PI / 2.0));
        pointY = (getStep(lastY, euler.z())) * (RESOLUTION_Y / (PI / 2.0));
